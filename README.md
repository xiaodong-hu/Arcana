...
Agents interact with authority by emitting JSONL requests. Arcana-Agent detects those request lines, asks the human to approve/edit/abort privileged operations in the TUI, relays approved requests to the session authority socket, shows shell execution in an embedded `[Arcana Run]` panel and other authority operations in an `[Arcana Request]` panel, and returns the JSON responses to the model:
